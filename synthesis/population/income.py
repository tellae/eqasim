...
def execute(context):
    ...
    df_households_size = (
        df_population
        .groupby(["household_id"], as_index=False)["person_id"].count()
    )
    df_households_size.rename(columns={"person_id": "household_size"}, inplace=True)
    df_households_size["household_size"] = (
        df_households_size["household_size"]
        .map(lambda x: str(x) + "_pers" if x < 5 else "5_pers_or_more")
    )
    ...
    df_households_info = df_population\
    ...
    df_households_single_man = df_households_info.query("household_size==1 and sex=='male'").reset_index()
    df_households_single_man["household_type"] = "Single_man"
    df_households_single_woman = df_households_info.query("household_size==1 and sex=='female'").reset_index()
    df_households_single_woman["household_type"] = "Single_woman"

    # compute couple without child
    # 2 persons and all have couple=True
    df_households_couple = df_population\
        .query("couple==True")\
        .groupby(["household_id"], as_index=False)\
        .agg(
            {
    ...
    df_households_couple_without_child = df_households_couple.query("couple==2 and household_size==2").reset_index()
    ...
    df_households_child = df_population\
        .query("age<25 and couple==False")\
        .groupby(["household_id"], as_index=False)\
        .agg(
            {
                "person_id": "count"
    ...
    df_households_couple_with_child = df_households_couple\
        .merge(df_households_child, how="left", on="household_id")\
    ...
    df_households_no_couple = df_population\
        .query("couple==False and household_size>=2")\
        .sort_values(["household_id","age"], ascending=[True, False])

    df_households_no_couple_oldest = df_households_no_couple\
        .groupby(["household_id"], as_index=False)\
        .agg({"age": "first", "person_id": "first"})\
        .rename(columns={"age": "oldest_age", "person_id": "oldest_person_id"})

    df_households_no_couple_second_oldest = df_households_no_couple\
        .merge(df_households_no_couple_oldest, how="left")\
    ...
    df_households_single_parent = df_households_no_couple_oldest\
        .merge(df_households_no_couple_second_oldest, how="left", on="household_id")
    df_households_single_parent = df_households_single_parent\
        .query("oldest_age>=25 and oldest_age<60 and second_oldest_age<25")\
        .reset_index()
    df_households_single_parent["household_type"] = "Single_parent"

    # combine type
    df_households = df_households\
        .merge(df_households_single_man[["household_id", "household_type"]], how="left", on="household_id", suffixes=('', '_1'))\
        .merge(df_households_single_woman[["household_id", "household_type"]], how="left", on="household_id", suffixes=('_1', '_2'))\
        .merge(df_households_couple_without_child[["household_id", "household_type"]], how="left", on="household_id", suffixes=('_2', '_3'))\
        .merge(df_households_couple_with_child[["household_id", "household_type"]], how="left", on="household_id", suffixes=('_3', '_4'))\
        .merge(df_households_single_parent[["household_id", "household_type"]], how="left", on="household_id", suffixes=('_5', '_5'))
    df_households.columns = ["household_id", "consumption_units", "commune_id", "household_size",
                             "household_type_1", "household_type_2", "household_type_3",
                             "household_type_4", "household_type_5"]
    df_households[["household_type_1", "household_type_2", "household_type_3",
                   "household_type_4", "household_type_5"]] = df_households[["household_type_1", "household_type_2", "household_type_3",
                   "household_type_4", "household_type_5"]].fillna("")
    df_households["household_type"] = \
        df_households.apply(
            lambda x: str(x["household_type_1"]) + str(x["household_type_2"]) + str(x["household_type_3"]) + str(x["household_type_4"]) + str(x["household_type_5"]),
            axis=1)
    # compute complex households as other households
    df_households["household_type"] = df_households["household_type"].map(lambda x: "complex_hh" if x == "" else x)

    df_households = df_households[["household_id", "consumption_units", "commune_id", "household_size", "household_type"]]
